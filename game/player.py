...
class Player:
    ...
    world_location: str = "dormitory"  # 当前所在世界地点
    npc_affinity: dict[str, int] = field(default_factory=dict)  # NPC 好感度（0~100）
    story_flags: dict[str, str] = field(default_factory=dict)  # 剧情与活动状态
    sect_contribution: int = 0                                # 宗门贡献
    encounter_log: list[str] = field(default_factory=list)    # 已完成奇遇图鉴
    ...
    technique_progress: dict[str, int] = field(default_factory=dict)  # 功法熟练度 0~100
    technique_level: dict[str, int] = field(default_factory=dict)     # 功法等级 1~4
    ...
    def add_npc_affinity(self, npc_id: str, value: int) -> int:
        """调整 NPC 好感度并钳制到 [0, 100]，返回新值。"""
        current = int(self.npc_affinity.get(npc_id, 0))
        updated = max(0, min(100, current + int(value)))
        self.npc_affinity[npc_id] = updated
        return updated

    def npc_affinity_for(self, npc_id: str) -> int:
        return max(0, min(100, int(self.npc_affinity.get(npc_id, 0))))
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "technique_progress": dict(self.technique_progress),
            "technique_level": dict(self.technique_level),
    ...
    def from_dict(cls, data: dict) -> "Player":
        return cls(
            ...
            world_location=data.get("world_location", "dormitory"),
            npc_affinity={str(k): int(v) for k, v in data.get("npc_affinity", {}).items()},
            story_flags={str(k): str(v) for k, v in data.get("story_flags", {}).items()},
            sect_contribution=int(data.get("sect_contribution", 0)),
            ...
            technique_progress={str(k): int(v) for k, v in data.get("technique_progress", {}).items()},
            technique_level={str(k): int(v) for k, v in data.get("technique_level", {}).items()},
