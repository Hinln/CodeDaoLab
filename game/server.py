...
from . import achievements, ai, config, curriculum, dialogues, items, npcs, progression, sandbox, save, secret_realms, story, tasks, techniques, world
...
def _build_state(player: Player) -> dict:
    ...
    tech_list = techniques.summarize(player)
    item_list = []
    for item_id, item in items.items().items():
        item_list.append(dict(item, id=item_id, count=player.items.get(item_id, 0)))
    return {
        "player": player.to_dict(),
        "world": world.summarize(player),
        "npcs": npcs.all_npcs(player),
        "story": story.summarize(player),
        "progression": progression.summarize(player),
        "realm": {
            "key": realm["key"],
            "name": realm["name"],
            "order": realm["order"],
...
def create_app() -> Flask:
    ...
    def api_world():
        with _state_lock:
            ...
            return jsonify({"world": world.summarize(player)})

    def _move_world_player(player: Player, location_id: str):
        if not location_id:
            return jsonify({"error": "缺少目标地点"}), 400
        allowed, reason = world.location_entry_status(player, location_id)
        if not allowed:
            return jsonify({"error": reason}), 403
        player.world_location = location_id
        player.touch()
    ...
    def api_world_move():
        with _state_lock:
            ...
            return _move_world_player(player, str(data.get("location_id", "")).strip())

    @app.post("/api/world/location/<location_id>/enter")
    def api_world_enter(location_id: str):
        with _state_lock:
            player = _require_player()
            if player is None:
                return jsonify({"error": "尚未创建角色"}), 400
            return _move_world_player(player, location_id)
    ...
    def api_npcs():
        """NPC 列表。"""
        with _state_lock:
            return jsonify({"npcs": npcs.all_npcs(_require_player())})
    ...
    def api_techniques():
        ...
        with _state_lock:
            ...
            if player is None:
                return jsonify({"error": "尚未创建角色"}), 400
            return jsonify({"techniques": techniques.summarize(player)})

    def _require_training_technique(player: Player, technique_id: str):
        technique = techniques.get_technique(technique_id)
        if technique is None:
            return None, (jsonify({"error": "功法不存在"}), 404)
        if technique_id not in player.techniques:
            return None, (jsonify({"error": "尚未习得该功法"}), 403)
        if player.world_location != "training_ground":
            return None, (jsonify({"error": "需前往演武场"}), 403)
        return technique, None

    @app.get("/api/techniques/<technique_id>/challenge")
    def api_technique_challenge(technique_id):
        with _state_lock:
            player = _require_player()
            if player is None:
                return jsonify({"error": "尚未创建角色"}), 400
            _technique, error = _require_training_technique(player, technique_id)
            if error:
                return error
            return jsonify({"challenge": techniques.public_challenge(technique_id)})

    @app.post("/api/techniques/<technique_id>/practice")
    def api_technique_practice(technique_id):
        with _state_lock:
            player = _require_player()
            if player is None:
                return jsonify({"error": "尚未创建角色"}), 400
            ...
            outcome = techniques.validate_challenge(technique_id, code)
            if outcome is None:
                return jsonify({"error": "功法缺少修炼挑战"}), 400
            gained = 0
            if outcome.passed:
                gained = techniques.add_progress(player, technique_id, 30)
                save.save_game(player, _current_slot)
            return jsonify({"outcome": outcome.to_dict(), "progress_gained": gained, "state": _build_state(player)})

    @app.post("/api/techniques/<technique_id>/exam")
    def api_technique_exam(technique_id):
        with _state_lock:
            player = _require_player()
            if player is None:
                return jsonify({"error": "尚未创建角色"}), 400
            ...
            if error:
                return error
            if not techniques.can_exam(player, technique_id):
                return jsonify({"error": "熟练度尚未达到考核要求"}), 403
            code = (request.get_json(silent=True) or {}).get("code") or ""
            outcome = techniques.validate_challenge(technique_id, code)
            level = int(player.technique_level.get(technique_id, 1))
            if outcome and outcome.passed:
                level = techniques.complete_exam(player, technique_id)
                save.save_game(player, _current_slot)
            return jsonify({"outcome": outcome.to_dict(), "level": level, "state": _build_state(player)})
    ...
    def api_story():
        with _state_lock:
            ...
            if player is None:
                return jsonify({"error": "尚未创建角色"}), 400
            return jsonify({"story": story.summarize(player)})
    ...
    def api_progression():
        with _state_lock:
            ...
            if player is None:
                return jsonify({"error": "尚未创建角色"}), 400
            return jsonify({"progression": progression.summarize(player)})

    @app.get("/api/story/quests/<quest_id>")
    def api_story_quest(quest_id):
        with _state_lock:
            player = _require_player()
            if player is None:
                return jsonify({"error": "尚未创建角色"}), 400
            if player.world_location != "mission_hall":
                return jsonify({"error": "需前往任务堂"}), 403
            quest = story.public_quest(quest_id)
            if quest is None:
                return jsonify({"error": "宗门任务不存在"}), 404
            return jsonify({"quest": quest})

    @app.post("/api/story/quests/<quest_id>/submit")
    def api_story_quest_submit(quest_id):
        with _state_lock:
            player = _require_player()
            if player is None:
                return jsonify({"error": "尚未创建角色"}), 400
            ...
            code = (request.get_json(silent=True) or {}).get("code") or ""
            try:
                outcome, reward = story.submit_quest(player, quest_id, code)
            except ValueError as error:
                return jsonify({"error": str(error)}), 400
            if outcome.passed:
                save.save_game(player, _current_slot)
            return jsonify({"outcome": outcome.to_dict(), "reward": reward, "state": _build_state(player)})

    @app.post("/api/story/encounters/trigger")
    def api_story_encounter_trigger():
        with _state_lock:
            player = _require_player()
            if player is None:
                return jsonify({"error": "尚未创建角色"}), 400
            try:
                encounter = story.trigger_encounter(player, player.world_location)
            except ValueError as error:
                return jsonify({"error": str(error)}), 403
            save.save_game(player, _current_slot)
            return jsonify({"encounter": story.public_encounter(encounter), "state": _build_state(player)})

    @app.post("/api/story/encounters/<encounter_id>/submit")
    def api_story_encounter_submit(encounter_id):
        with _state_lock:
            player = _require_player()
            if player is None:
                return jsonify({"error": "尚未创建角色"}), 400
            code = (request.get_json(silent=True) or {}).get("code") or ""
            try:
                outcome, reward = story.submit_encounter(player, encounter_id, code)
            except ValueError as error:
                return jsonify({"error": str(error)}), 400
            if outcome.passed:
                save.save_game(player, _current_slot)
