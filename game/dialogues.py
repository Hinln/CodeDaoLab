...
def apply_actions(player, actions: list[dict]) -> list[str]:
...
def choose(player, npc_id: str, node_id: str, option_index: int) -> dict:
    ...
    if target == END_NODE:
        return {"npc_id": npc_id, "node_id": END_NODE, "text": "", "options": [], "effects": [], "ended": True}
    target_node = get_node(npc_id, target) or {}
    effects = apply_actions(player, target_node.get("actions", []))
    return {
        "npc_id": npc_id,
        "node_id": target,
