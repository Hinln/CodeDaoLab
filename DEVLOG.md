...
## V0.3-A 世界地图系统（2026-08-08）

### 完成内容

- `world.json` 扩展为青云宗、青云城、后山秘境 3 个区域，共 14 个地点；每个地点包含坐标、双向连接和数据驱动解锁条件。
- 世界移动升级为连通图校验：目标地点必须已解锁且与当前位置直接相连，旧档地点失效时安全回退洞府。
- 新增正式接口 `POST /api/world/move`，保留旧地点进入接口作为兼容别名；`GET /api/world` 返回当前位置、解锁与可达状态。
- 前端地图显示区域开放状态、地点连通去向、当前位置以及不可移动原因，移动调用切换至正式接口。
- 存档版本从 2 升级为 3，V0.1/V0.2 缺失地图字段的存档继续通过默认值加载。

### 测试与修复

- 新增 `tests/test_v03_world.py` 6 项：区域/地点规模、连接合法与对称、相邻可达、解锁与移动校验、API 移动、V0.2 存档迁移。
- pytest 全量：`245 passed, 32 skipped`。
- E2E：新增洞府→山门→洞府地图移动，完整浏览器流程全部通过。
- 修复 E2E 第一次移动后成功提示遮挡第二次点击的问题：测试流程按产品交互关闭提示后继续。

### 阶段结论

- V0.3-A 全部任务与验收项完成，可以进入阶段 B（NPC 生命化系统）。

## V0.3-B NPC 生命化系统（2026-08-08）

### 完成内容

- 青玄子、玄机长老、云游子、青云真人（掌门）四位核心 NPC 完成地点绑定、好感度、关系称谓与专属高好感剧情；保留丹尘子、小月既有内容。
- NPC 地点统一改用世界地图 ID；新增掌门并安置于任务堂，全部 6 位 NPC 均能映射到有效地点。
- Player 新增 `npc_affinity`，支持 0~100 钳制、增减、序列化与旧档默认兼容。
- 对话系统新增数据驱动 `affinity` 动作与 `require` 条件；好感动作沿用对话幂等标记，重复对话不会刷取奖励。
- NPC API、世界状态和对话响应均返回好感度、关系称谓与在场状态。
- 取消固定 NPC 横条；NPC 进入地点卡片，玩家必须移动至其所在地点后才能从前端交谈；对话弹窗新增关系与好感度进度条。

### 测试与修复

- 新增 `tests/test_v03_npcs.py` 6 项：核心 NPC 数据、地点合法性、好感钳制与存档、幂等奖励、专属剧情门槛、API 与世界在场状态。
- B 专项与旧 NPC 架构测试：`22 passed`。
- pytest 全量：`251 passed, 32 skipped`。
- E2E：验证固定横条已移除、青玄子洞府交互、移动至藏经阁解锁支线、沿山道到秘境入口寻找云游子；完整流程全部通过。
- 修复旧 E2E 远程点击云游子的问题：改为按世界连接逐点移动，确认地点限制真实生效。

### 阶段结论

- V0.3-B 全部任务与验收项完成，可以进入阶段 C（功法修炼系统）。

## V0.3-C 功法修炼系统（2026-08-08）

### 完成内容

- 新增 `game/data/techniques.json`，13 门独立功法覆盖输出、变量、类型、分支、循环、函数、列表、字典、异常、模块、文件与 OOP。
- Player 新增 `technique_progress` 与 `technique_level`，支持旧档默认兼容；功法等级为入门/小成/大成/圆满，熟练度阈值为 0/40/70/100。
- 功法树以前置功法等级描述；境界突破继续自动习得对应功法，筑基可同时习得列表与字典功法。
- 已习得功法在真实任务通过后获得熟练度；演武场修炼每次也必须通过真实沙箱代码验证。
- 熟练度达标不会自动升级，玩家必须在演武场再次通过代码考核；大成时获得悟性反馈。
- 新增演武场 API、地图地点入口、修炼挑战编辑器、熟练度与考核 UI；藏经阁同步展示功法等级和熟练度。

### 测试与修复

- 新增 `tests/test_v03_techniques.py` 6 项；13 份功法参考解全部由真实沙箱验证。
- C 专项与旧功法架构测试：`22 passed`。
- pytest 全量：`257 passed, 32 skipped`。
- E2E：玩家移动至演武场，两次提交真言诀代码获得熟练度，再通过代码考核晋升小成；完整流程全部通过。
- 修复功法输出检查简写适配，以及 E2E 考核挑战异步加载覆盖输入的竞态。

### 阶段结论

- V0.3-C 全部任务与验收项完成，可以进入阶段 D（剧情与奇遇系统）。

## V0.3-D 剧情与奇遇系统（2026-08-08）

### 完成内容

- 新增 `game/data/story.json` 与 `game/story.py`：3 章主线纪事、5 个宗门支线、8 种随机奇遇。
- 章节节点挂接已有真实代码任务；宗门支线与奇遇各自携带独立代码挑战，统一复用成熟任务验证器和沙箱。
- 新增任务堂纪事界面，以及山道/客栈/奇遇点的探索入口；触发事件不产生奖励，代码验证通过后才结算修为、宗门贡献和图鉴。
- Player 新增 `story_flags`、`sect_contribution`、`encounter_log`，当前奇遇可随存档恢复。
- 新增剧情、宗门任务与奇遇 API；重复任务、错误奇遇和非对应地点均受服务端校验。

### 测试与修复

- 新增 `tests/test_v03_story.py` 6 项；5 个支线与 8 个奇遇参考解全部经真实沙箱验证。
- pytest 全量：`263 passed, 32 skipped`。
- E2E：移动至奇遇点，成功触发随机奇遇代码挑战，再继续秘境与完整通关流程；全部通过。

### 阶段结论

- V0.3-D 全部任务与验收项完成，可以进入阶段 E（长期成长闭环）。

## V0.3-E 长期成长闭环（2026-08-08）

### 完成内容

- 新增只读 `progression` 聚合层与 `/api/progression`，统一展示宗门贡献、功法图鉴、奇遇图鉴、NPC 关系和自由目标。
- 自由目标覆盖主线、功法、探索、关系、宗门五类，并明确指向洞府、演武场、奇遇点、NPC 所在地和任务堂。
- 前端新增“道途总览”入口与响应式面板，展示 13 门功法、8 种奇遇、四位核心 NPC 关系和当前成长统计。
- 聚合层无任何写入或直接增长接口；所有成长继续来自真实代码任务、修炼、考核、支线与奇遇结算。

### 测试与修复

- 新增 `tests/test_v03_progression.py` 5 项：系统齐全、成长状态映射、地点合法、只读性、API。
- pytest 全量：`268 passed, 32 skipped`。
- E2E：新角色道途总览显示功法 `1/13`、奇遇 `0/8`、五类自由目标与四段核心关系；完整流程全部通过。

### 阶段结论

- V0.3-E 全部任务与验收项完成，可以进入阶段 F（最终验收与发布）。

## 2026-08-08 - V0.3 F 阶段：最终验收与发布

### 完成内容

- 按 A → B → C → D → E → F 顺序完成 V0.3 全部阶段。
- 将旧功能卡片菜单退出可见界面，所有玩法入口迁移到世界地点或顶部工具栏。
- 更新版本号、README、ROADMAP、CHANGELOG、构建脚本和端到端流程。
- 生成 V0.3 验收报告与测试报告。
- 构建 `dist/码上飞升-v0.3.0.pyz`（172 KB）与 `dist/启动游戏.bat`。

### 验收结果

- JavaScript 语法检查：通过。
- Python：268 passed，32 skipped，0 failed。
- E2E：ALL E2E CHECKS PASSED。
- zipapp 冒烟验证：9 / 9 通过。

### 阶段结论

F 阶段完成，V0.3.0 达到发布条件。
